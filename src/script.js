...
const badgeTargets = {
  ...
  hoveredUserDMM: 'css-901oao r-vlxjld r-xoduu5 r-18u37iz r-1q142lx r-37j5jr r-1inkyih r-16dba41 r-rjixqe r-bcqeeo r-qvutc0',
...
function isBadgeElement (elementClass, targets) {
  let isElementExist = false
  for (const key in targets) {
    /* eslint-disable no-prototype-builtins */
    if (targets.hasOwnProperty(key) && targets[key] === elementClass) {
      isElementExist = true
      break
    }
  }
...
function findElement (element) {
  if (isBadgeElement(element.className, badgeTargets)) {
...
function changeToTwitterBlueBadge (element) {
  ...
  if (element.childNodes) {
    [...element.childNodes].forEach(changeToTwitterBlueBadge)
  }
}
...
function userVerifyStatus (element) {
  const mainReactProps = getReactProps(element)
  const isBlueVerified = mainReactProps.children.props.children[0][0].props.isBlueVerified
  const isVerified = mainReactProps.children.props.children[0][0].props.isVerified

  if (!isVerified) {
    if (isBlueVerified) {
      changeToTwitterBlueBadge(element)
    }
  }
}
